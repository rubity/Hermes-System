...
type NanoData struct {
	AppendNumber string `json:"@appendNumber"`
	Mode         string `json:"@mode"`
	ScanAxis     string `json:"@scanAxis"`
	Status       string `json:"@status"`
	DataPoint    struct {
		CommonCountingTime struct {
	...
	Header struct {
		...
		Source       struct {
			ApplicationSoftware struct {
				Text    string `json:"#text"`
				Version string `json:"@version"`
			}
			InstrumentControlSoftware struct {
				Text    string `json:"#text"`
				Version string `json:"@version"`
			}
...
func (s *SmartContract) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
	fmt.Println("Entrei na função invoke")
...
func (s *SmartContract) initLedger(stub shim.ChaincodeStubInterface) sc.Response {
	datas := []Data{
		Data{ID: "111", Sensor: "teste1", Value: 111, Timestamp: 111, Location: "teste1", PhysicalQuantity: "teste1"},
		Data{ID: "222", Sensor: "teste2", Value: 222, Timestamp: 222, Location: "teste2", PhysicalQuantity: "teste2"},
	}

	i := 0
	for i < len(datas) {
		fmt.Println("i is ", i)
		carAsBytes, _ := json.Marshal(datas[i])
